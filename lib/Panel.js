...
exports.init = function(){
    ...
    //initialize prefs before panel so we can preload panel before the user clicksp it
    ...
    function setupPanel(){
        ...
        panel.port.emit("setCheckBox","pdfjs",PrefServ.getter("extensions.agentSpoof.pdfjs"));
        panel.port.emit("setCheckBox","search_suggest",PrefServ.getter("extensions.agentSpoof.searchsuggest"));
    ...
    panel.port.once("exclusion_count",function(exclusionObj){

        //TODO sort out exclude counts

        //store exclusion count
        PrefServ.setter("extensions.agentSpoof.exclusionCount", JSON.stringify(exclusionObj));

        //retrive it, change a vlaue and re store it
        //var y = JSON.parse(PrefServ.getter("extensions.agentSpoof.exclusionCount"));
        //y["desktop"].total_count = 10;
        //console.log(y["desktop"].total_count);
        //PrefServ.setter("extensions.agentSpoof.exclusionCount",JSON.stringify(y));
    ...
    panel.port.on("excludecb",function(profileid,profileindices,checked){
        ...
        if(checked == true){
            ...
            else{ // a mobile, console, etc profile
                prof["non_desktop"].exclude_count++;
            }
