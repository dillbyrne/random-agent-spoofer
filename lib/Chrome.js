	...
	Observer =
{
	observe: function(subject, topic, data)
	{
		...
		}else if (data === "extensions.agentSpoof.timeInterval" || data === "extensions.agentSpoof.uaChosen" ){
		...
		if (topic == "http-on-modify-request") {
			...
			for (i = 0, len = whiteListStateArray.length ; i < len ; i++) {
				whiteListStateArray[i] = false;
			}

			//if the whitelist is not disabled and the default profile is not in use
			if(PrefServ.getter("extensions.agentSpoof.whiteListDisabled") == false && PrefServ.getter("extensions.agentSpoof.uaChosen") != "default"){
