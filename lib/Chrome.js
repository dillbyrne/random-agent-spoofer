    ...
    observe: function(subject, topic, data)
    {
        ...
        if (topic == "http-on-modify-request") {
            ...
            if(PrefServ.getter("extensions.agentSpoof.enabled") == true){
                ...
                if(PrefServ.getter("extensions.agentSpoof.authorization") == true && whiteListStateArray[1] == false){
                    httpChannel.setRequestHeader("Authorization","",false);
                }

				//set spoofed xff header ip				
                if(PrefServ.getter("extensions.agentSpoof.xff") == true && whiteListStateArray[2] == false){
				...
				if(PrefServ.getter("extensions.agentSpoof.via") == true && whiteListStateArray[3] == false){
				...
				if (PrefServ.getter("extensions.agentSpoof.ifnone") == true && whiteListStateArray[4] == false){
...
function checkWhiteListConfig(index){
    ...
    //check if authorization header has been whitelisted to allow it
    if(whiteListItem.authorization === true)
