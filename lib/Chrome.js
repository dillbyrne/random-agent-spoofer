...
var PrefServ = require('./PrefServ'),
	...
	Observer = {
		...
		observe: function(subject, topic, data) {
			...
			if (data === 'extensions.jid1-AVgCeF1zoVzMjA@jetpack.enabled') {
					
				if(PrefServ.getter(data) === true){
					PrefServ.toggleOn();
				} else {
					PrefServ.toggleOff();
				}
				...
				panel.port.emit('toggleState', !PrefServ.getter(data));
				
			} else if (data === 'extensions.jid1-AVgCeF1zoVzMjA@jetpack.show_context_menu') {

				ContextMenu.showMenu(PrefServ.getter('extensions.jid1-AVgCeF1zoVzMjA@jetpack.show_context_menu'));
			
			} else if (data === 'extensions.agentSpoof.whiteListDisabled'){
