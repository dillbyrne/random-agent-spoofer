...
//from 0.9.5.2(AMO) or 0.9.5.3 to 0.9.5.4 only
exports.onUpgrade = function() {

	//upgrade to the latest firefox if the previous default whilelist profile is in use
	if(PrefServ.getter("extensions.agentSpoof.whiteListUserAgent") === "Mozilla/5.0 (Windows NT 6.2; rv:37.0) Gecko/20100101 Firefox/37.0")
		PrefServ.setter("extensions.agentSpoof.whiteListUserAgent","Mozilla/5.0 (Windows NT 6.2; rv:43.0) Gecko/20100101 Firefox/43.0");
	...
	//handle change in referer pref
	if (PrefServ.getter('extensions.agentSpoof.referer') === 'none'){
		PrefServ.setter('extensions.agentSpoof.disableRef',true);
		(require('./Panel').getPanel()).port.emit('setCheckBox', 'ref', true);
	}
	else
		PrefServ.setter('extensions.agentSpoof.disableRef',false);

	//remove old prefs
	PrefServ.resetter('extensions.agentSpoof.referer');
	PrefServ.resetter('plugins.enumerable_names');
};
