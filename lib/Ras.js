    ...
    //random platfrom specific options e.g random windows browsers
        ...
        if (excludeCount[uaChosen].exclude_count < excludeCount[uaChosen].total_count){
            ...
            ttip = setProfile(indices[0],indices[1],profileRandNum,true,
                "Spoofing enabled\nUncheck the spoofing option to disable or use the context menu option\nRandom "+profData[indices[0]].list[indices[1]].description+":\n",
                "Browser Profile Changed\nRandom "+profData[indices[0]].list[indices[1]].description+":\n");

           changed = true;

        }else{
            Notifications.sendMsg("Unable to randomly change "+profData[indices[0]].list[indices[1]].description+
                " Profile\nAll "+profData[indices[0]].list[indices[1]].description+
