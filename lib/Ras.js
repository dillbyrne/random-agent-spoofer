...
function prepareAndSetAgentInfo(){ 
    ...
    if(PrefServ.getter("extensions.agentSpoof.uaChosen") == "random"){
        ...
        if ( PrefServ.getter("extensions.agentSpoof.randomOtherCount") + PrefServ.getter("extensions.agentSpoof.randomDesktopCount")
         < PrefServ.getter("extensions.agentSpoof.totalRandomAllCount") ){
           ...
           //check if the randomly chosen profile is in the exclude list
            ...
            setAgentInfo(listTypeRandNum,listRandNum,profileRandNum);
        
            //save the current random indices
            cur_rand = listTypeRandNum+","+listRandNum+","+profileRandNum;

            //set the tooltip
            var uainfo = profData[listTypeRandNum].list[listRandNum].useragents[profileRandNum].description;
        ...
        if (PrefServ.getter("extensions.agentSpoof.randomDesktopCount") < PrefServ.getter("extensions.agentSpoof.totalRandomDesktopCount")){
            ...
            //set the random user agent
            setAgentInfo(deskIndex,listRandNum,profileRandNum);

            //set the tooltip
            var uainfo = profData[deskIndex].list[listRandNum].useragents[profileRandNum].description;

            //save the current random indices
            cur_rand = deskIndex+","+listRandNum+","+profileRandNum;
        ...
        //set the user agent
        setAgentInfo(indices[0],indices[1],indices[2]);

        //set the tooltip
        var uainfo = profData[indices[0]].list[indices[1]].useragents[indices[2]].description;
...
//for the upgrade to 0.9.5 only
exports.onUpgrade = function(){

    //change random to random_time
    if(PrefServ.getter("extensions.agentSpoof.timeInterval") === "random")
        PrefServ.setter("extensions.agentSpoof.timeInterval","random_time");

    //remove old profile references
    PrefServ.setter("extensions.agentSpoof.excludeList","");
    PrefServ.setter("extensions.agentSpoof.randomDesktopCount",0);
    PrefServ.setter("extensions.agentSpoof.randomOtherCount",0);
