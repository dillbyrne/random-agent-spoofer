...
exports.init = function() {
    ...
    //set initial profile and timer options if the user had the addon enabled on shutdown
    ...
    // check for updates if the user selected the option
    // this is for the github version only
    if( PrefServ.getter("extensions.jid1-AVgCeF1zoVzMjA@jetpack.check_for_updates") === true){
        // delay checking for updates by 5 seconds 
        // to allow the addon to start up
        Timer.setOneTimeTimeout( Req.checkForUpdates ,5000 );
    }
};
...
function prepareAndSetAgentInfo(){ 
    ...
    if( uaChosen == "random"){
        ...
        if ( excludeCount["non_desktop"].exclude_count + excludeCount["desktop"].exclude_count 
            < excludeCount["non_desktop"].total_count + excludeCount["desktop"].total_count ){
           ...
           //check if the randomly chosen profile is in the exclude list
            ...
            ttip = setProfile(listTypeRandNum,listRandNum,profileRandNum,true,
                ...
                +localization.getString("uncheck_spoofing_id")+"\n"
        ...
        ttip =  localization.getString("spoofing_enabled_id")+"\n"
                +localization.getString("uncheck_spoofing_id")+"\n"
