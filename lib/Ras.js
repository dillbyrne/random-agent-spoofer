...
function prepareAndSetAgentInfo() {
	...
	if (uaChosen == "random") {
		...
		if (excludeCount["other"].exclude_count + excludeCount["mobile"].exclude_count + excludeCount["desktop"].exclude_count
			...
			Notifications.sendMsg(
				localization.getString("unable_to_change_random_profile_id") + "\n"
		...
		if (excludeCount["desktop"].exclude_count < excludeCount["desktop"].total_count) {
			...
			ttip = setProfile(
				...
				localization.getString("spoofing_enabled_id") + "\n"
					+ localization.getString("disable_spoofing_id") + "\n"
					+ localization.getString("random_desktop_id") + ": ",
				localization.getString("browser_profile_changed_id")+"\n"
					+ localization.getString("random_desktop_id") + ": "
			);
			...
			Notifications.sendMsg(
				localization.getString("unable_to_change_random_desktop_profile_id") + "\n"
		...
		if (excludeCount["mobile"].exclude_count < excludeCount["mobile"].total_count) {
			...
			ttip = setProfile(
				...
				localization.getString("spoofing_enabled_id") + "\n"
					+ localization.getString("disable_spoofing_id") + "\n"
					+ localization.getString("random_mobile_id") + ": ",
				localization.getString("browser_profile_changed_id") + "\n"
					+ localization.getString("random_mobile_id") + ": "
			);
			...
			Notifications.sendMsg(
				localization.getString("unable_to_change_random_mobile_profile_id") + "\n"
	...
	//random platfrom specific options e.g random windows browsers
		...
		if (excludeCount[uaChosen].exclude_count < excludeCount[uaChosen].total_count) {
			...
			Notifications.sendMsg(
				localization.getString("unable_to_randomly_change_id") + " "
					+ profData[indices[0]].list[indices[1]].description + " "
...
exports.onInstall = function() {
	...
	PrefServ.setter("network.http.use-cache", false); //set as disabled by default as a fix for #121
	PrefServ.setter("plugins.enumerable_names", "");
...
exports.onUpgrade = function() {
	...
	if (PrefServ.getter("extensions.agentSpoof.fullWhiteList") === "[{\"url\":\"youtube.com\",\"options\":[\"canvas\"]}]")
		...
		PrefServ.setter(
			...
			"[{\"url\":\"play.google.com\"},{\"url\":\"s.ytimg.com\",\"options\":[\"canvas\"]},{\"url\":\"youtube.com\",\"options\":[\"canvas\",\"windowName\"]}]"
		);
	...
	//set dnt and tab history prefs if enabled in our old pref values before
	//they are removed since they are now handled by different prefs
	if (PrefServ.getter("extensions.agentSpoof.tabHistory") === true)

		PrefServ.setter("extensions.agentSpoof.limitTab", true);

	if (PrefServ.getter("extensions.agentSpoof.dnt") === "enabled")

		PrefServ.setter("privacy.donottrackheader.enabled", true);
	...
	PrefServ.resetter("browser.sessionhistory.max_entries"); //now in inject script so users can still use it but sites reading it get 2
