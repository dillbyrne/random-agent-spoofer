...
exports.onUpgrade = function(){
    ...
    if(PrefServ.getter("extensions.agentSpoof.whiteListUserAgent") === "Mozilla/5.0 (Windows NT 6.2; rv:35.0) Gecko/20100101 Firefox/35.0")
        PrefServ.setter("extensions.agentSpoof.whiteListUserAgent","Mozilla/5.0 (Windows NT 6.2; rv:37.0) Gecko/20100101 Firefox/37.0");
...
function setAgentInfo(listTypeIndex,listIndex,profileIndex){
    ...
    for (key in profData[listTypeIndex].list[listIndex].useragents[profileIndex]) {
...
function createPlatfromContextMenuItems(){
    ...
    for (k = 0, len = profData.length ; k < len ; k++) {

        for (i = 0, len2 = profData[k].list.length ; i < len2 ; i++) {
