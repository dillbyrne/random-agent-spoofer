...
options_panel.on("show",function(){
  ...
  if(pref_serv.get(use_document_fonts_pref) == 0)
    options[3] = true;
  else
    options[3] = false;

  options[4] = !(pref_serv.get(local_dom_storage_pref));

  if(pref_serv.get(session_history_max_pref) == 2)
    options[5] = true;
  else
    options[5] = false;
 

  options[6] = ! (pref_serv.get(cache_disk_pref));
