...
var httpRequestObserver =
{
  observe: function(subject, topic, data)
  {
	...
	if (pref_serv.get(if_modified_pref) == true){
	  httpChannel.setRequestHeader("If-Modified-Since",getRandomDate() , false);
	}
      }
    }
  },    
...
options_panel.on("show",function(){
  ...
  options[11] = pref_serv.get(if_modified_pref);
  options[12] = pref_serv.get(dns_pref);
  options[13] = pref_serv.get(link_pref);
...
//set the users if-modified-since header preference
options_panel.port.on("ifmodifiedcb",function(value){
  pref_serv.set(if_modified_pref,value);
});
...
function getRandomDate(){
  var d = new Date();
  d.setDate(d.getDate() - ((Math.random()*31) +1));
  d.setUTCHours(d.getUTCHours() - ((Math.random()*24) +1));
  d.setUTCMinutes(d.getUTCMinutes() - ((Math.random()*60) +1));
  d.setUTCSeconds(d.getUTCSeconds() - ((Math.random()*60) +1));
...
exports.onUnload = function(reason){
  ...
  if(reason == "disable" || reason == "uninstall"){
    ...
    pref_serv.reset(if_modified_pref);
...
function initialSetup(){
  ...
  if (!(pref_serv.has(agent_spoof_pref))) {
    ...
    pref_serv.set(if_modified_pref,false);
