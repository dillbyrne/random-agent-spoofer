...
var geo_pref = "extensions.agentSpoof.geo";
...
var httpRequestObserver =
{
  observe: function(subject, topic, data)
  {
	...
	//sent the accept language header ourselves to stop firefox from
	//overwriting the q value
	if(pref_serv.get(accept_lang_pref) == true && pref_serv.get(ua_choice_pref) != "default" ){
	  httpChannel.setRequestHeader("Accept-Language",pref_serv.get(accept_lang_override),false);
	}
      }
    }
  },    
...
options_panel.on("show",function(){
  ...
  options[16] = pref_serv.get(agent_spoof_pref);
...
options_panel.port.on("spoofcb",function(value){
  ...
  //set the icon and label accordingly
  toggleIconAndLabel();

});

//toggle headers preference
options_panel.port.on("togglehcb",function(value){
  pref_serv.set(toggle_headers_pref,value);
});

//toggle extras preference
options_panel.port.on("toggleecb",function(value){
  pref_serv.set(toggle_extras,value);
});

//set the users default (document) accept header preference
options_panel.port.on("acceptdcb",function(value){
  setAcceptHeader(accept_default_pref,accept_default_override,value);
});


//set the users encoding accept header preference
options_panel.port.on("acceptecb",function(value){
  setAcceptHeader(accept_encoding_pref,accept_encoding_override,value);
});


//set the users language accept header preference
options_panel.port.on("acceptlcb",function(value){
  setAcceptHeader(accept_lang_pref,accept_lang_override,value);
});
...
options_panel.port.on("uachange",function(ua_choice,interval){
  ...
  pref_serv.set(ua_choice_pref,ua_choice);
    ...
    if(pref_serv.get(agent_spoof_pref) == true){
      ...
      if(pref_serv.get(timer_interval_pref) != "none" && pref_serv.get(ua_choice_pref).slice(0,6) == "random"){
	startTimer();
      }else{
...
function setAcceptHeader(header_pref,header_override,spoofHeader){

  pref_serv.set(header_pref,spoofHeader);
  
  if(pref_serv.get(agent_spoof_pref) == true && pref_serv.get(ua_choice_pref) != "default"){
	...
	pref_serv.set(header_override,ua_list.uadata[indices[0]].useragents[indices[1]]["accept_"+header_pref.slice(28).toLowerCase()]);	
    }
    else
      pref_serv.reset(header_override);
  }
}
...
function prepareAndSetAgentInfo(){
  ...
  else if(pref_serv.get(ua_choice_pref) == "default"){
...
function initialSetup(){
  ...
  if (!(pref_serv.has(agent_spoof_pref))) {
    ...
    pref_serv.set(accept_default_pref,false);
    pref_serv.set(accept_encoding_pref,false);
