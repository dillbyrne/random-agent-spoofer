...
options_panel.port.on("chosen-options",function(options){
  ...
  if(pref_serv.get(timer_interval_pref) != options[0] || pref_serv.get(ua_choice_pref) != options[1] || pref_serv.get(notify_pref) != options[2]){

    //save options selected
    savePrefs(options[0],options[1],options[2]);
