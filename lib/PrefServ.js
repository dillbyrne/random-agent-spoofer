...
exports.createPrefs = function(){
	...
	if (!(PrefServ.has("extensions.agentSpoof.dnt"))){ //set dnt pref (none/0/1)

		if (PrefServ.get("privacy.donottrackheader.enabled") == true){
			PrefServ.set("extensions.agentSpoof.dnt",PrefServ.get("privacy.donottrackheader.value").toString());
		}
		else
			PrefServ.set("extensions.agentSpoof.dnt","none");
	}

	if(!(PrefServ.has("extensions.agentSpoof.referer"))){ //set referer pref (0/1/2 for now)
		PrefServ.set("extensions.agentSpoof.referer",PrefServ.get("network.http.sendRefererHeader").toString());
	}
